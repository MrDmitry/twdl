...
class Stream():
    ...
    def name(self):
        return ''.join(c if c in Utils.valid_chars else '_' for c in self.meta['channel']['status'])

    def __lt__(self, rhs):
        return self.meta['_id'] < rhs.meta['_id']

    def __eq__(self, rhs):
        return False if rhs is None else self.meta['_id'] == rhs.meta['_id']
